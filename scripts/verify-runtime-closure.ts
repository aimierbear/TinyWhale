...
const PYTHON_RUNTIME_MANIFEST = 'python/sdk-runtime/package.json'
...
/**
 * Check that the runtime manifest contains every required workspace peer,
 * and that the Python SDK runtime also contains every shipped-preset plugin.
...
export async function verifyRuntimeClosure(
  root: string,
  manifestPath = PYTHON_RUNTIME_MANIFEST,
  ...
  const checkShippedPresets = manifestPath === PYTHON_RUNTIME_MANIFEST
...
if (import.meta.main) {
  ...
  for (const relative of manifests) {
    const result = await verifyRuntimeClosure(root, relative)
    if (result.failures.length > 0) {
      const kind = relative === PYTHON_RUNTIME_MANIFEST
        ? 'preset plugins or required workspace peers'
        : 'required workspace peers'
      console.error(`verify-runtime-closure: ${kind} are missing from ${relative} dependencies:`)
      for (const failure of result.failures) console.error(`  ${failure}`)
      failed = true
      continue
    }
    if (relative === PYTHON_RUNTIME_MANIFEST) {
      console.log(
      ...
      console.log(
        `verify-runtime-closure: ${relative}: ${result.workspacePackageCount} workspace packages form a closed runtime dependency graph.`,
      )
    }
  }
  ...
  for (const presetPath of presetPaths) {
    ...
    for (const target of targets) {
      ...
      for (const plugin of activeBarePluginPackages(document, processPlatform)) {
        ...
        const listed = missing.get(key) ?? new Set<string>()
        listed.add(target)
        missing.set(key, listed)
      }
    }
  }
  failures.push(...[...missing.entries()].map(([chain, listed]) =>
    `${chain} (${[...listed].sort().join(', ')})`))
  return failures
}
