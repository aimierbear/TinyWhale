...
- **Only ordinary loop chunks, marked local compaction outputs, and `verifier/call` events are derivable** — a pure pre-chunk throw, a cancel/hang, an unmarked external summarizer call, or a verifier failure whose exact provider error must replay needs the `replay.override.json` sidecar. Replacement and patch forms affect only the primary session; child scripts still derive from their logs.
