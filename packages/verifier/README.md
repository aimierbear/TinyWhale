...
This family ports the llm-as-a-verifier pairwise reward and Probabilistic Pivot Tournament behind `ctx.verifier`, and exposes them to the model as `verify` and to humans as `/verify`.
...
| [`command-verify/`](command-verify/README.md) | Human `/verify` command: parallel candidates, majority vote or PPT, submit winner | registers on `ctx.commands` |
