...
命令输入和直接的状态/错误输出不会进入模型请求。命令派发时，被调用的命令行 `/verify <task>` 会作为一条立即可见的 `user/message` 追加到聊天表面，随后才启动候选子代理。候选选择完成后，一条 `Best verified result for: <task>` 加所选结果的后续用户消息唤醒模型。失败时，一条 `Verification failed for: <task>` 加失败详情的后续用户消息同样唤醒模型，因此会话不会无声停止。这两条后续消息都是普通 `user/message` 事件，并从会话日志重放。
