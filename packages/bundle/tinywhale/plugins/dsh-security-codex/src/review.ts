...
const SEVERITY_ORDER: readonly ScanSeverity[] = ['critical', 'high', 'medium', 'low']
 ...
 */
async function collectUnderRoot(
  root: string,
  relPrefix: string,
  limits: ReviewLimits,
...
async function openAndRead(
  abs: string,
  maxFileBytes: number,
): Promise<{ readonly content: Buffer; readonly size: number } | 'too-large'> {
  const handle = await open(abs, 'r')
  try {
    const { size } = await handle.stat()
    if (size > maxFileBytes) return 'too-large'
 ...
 */
export async function collectFiles(request: ReviewRequest, limits: ReviewLimits): Promise<CollectedFiles> {
  ...
  if (request.paths !== undefined && request.paths.length > 0) {
    ...
    for (const raw of request.paths) {
      ...
      if (link === undefined) throw new Error(`review path does not exist: ${raw}`)
 ...
 */
function renderFileBlock(file: SourceFile, budgetBytes: number): string {
 ...
 */
export function buildBatches(files: readonly SourceFile[], batchBytes: number): string[] {
...
function batchPrompt(batchText: string, target: string, extraInstructions?: string): string {
  const lines = [
...
function verifyPrompt(findings: ReviewFinding[], batchText: string, target: string): string {
  return [
...
async function collectResponse(stream: AsyncIterable<StreamChunk>): Promise<{
 ...
 */
export function parseFindings(text: string, maxFindings: number): ReviewFinding[] | undefined {
...
function normalizeFindings(value: unknown, maxFindings: number): ReviewFinding[] | undefined {
  let raw: unknown
  if (Array.isArray(value)) {
    raw = value
  } else if (value !== null && typeof value === 'object') {
    raw = (value as Record<string, unknown>).findings
  } else {
    return undefined
  }
  ...
  for (const entry of raw) {
    ...
    const severityIndex = (SCAN_SEVERITIES as readonly string[]).indexOf(severity)
    if (severityIndex < 0) continue
    if (typeof title !== 'string' || title.length === 0 || title.length > FINDING_TITLE_MAX) continue
    if (typeof path !== 'string' || path.length === 0 || path.length > FINDING_PATH_MAX) continue
    if (path.includes('\\') || path.startsWith('/') || path.split('/').includes('..')) continue
    if (typeof description !== 'string' || description.length === 0 || description.length > FINDING_TEXT_MAX) continue
    const line = candidate.line
    if (line !== undefined && (!Number.isInteger(line) || (line as number) < 1)) continue
    const suggestion = candidate.suggestion
    if (suggestion !== undefined && (typeof suggestion !== 'string' || suggestion.length === 0 || suggestion.length > FINDING_TEXT_MAX)) continue
    findings.push({
...
async function invokeOnce(
  ...
  system: string,
  userText: string,
): Promise<{ readonly text: string; readonly usage: TokenUsage | undefined; readonly truncated: boolean }> {
  ...
  switch (response.finishKind) {
    ...
    case 'error':
      throw new ReviewModelError(
        response.failureMessage ?? 'the model call failed',
        response.failureCode ?? 'LLM',
      )
    case 'aborted':
      throw new ReviewModelError(
        response.failureMessage ?? 'the model call was aborted',
        response.failureCode ?? 'ABORTED',
      )
    default:
...
function consolidate(findings: ReviewFinding[], maxFindings: number): ReviewFinding[] {
  const seen = new Set<string>()
  const unique: ReviewFinding[] = []
  for (const finding of findings) {
    if (unique.length >= maxFindings) break
 ...
 */
export async function runReview(options: ReviewEngineOptions): Promise<ReviewResult> {
  ...
  for (const batch of capped) {
    ...
    if (response.truncated) truncatedBatches += 1
  ...
  for (const batchFindings of perBatch) findings.push(...batchFindings)
  ...
  if (options.request.mode === 'deep') {
    for (let index = 0; index < capped.length && index < perBatch.length; index += 1) {
      ...
      if (firstPass === undefined || firstPass.length === 0) continue
      options.signal?.throwIfAborted()
      ...
      if (verified === undefined) unparseableBatches += 1
    }
    findings.length = 0
    for (const batchFindings of perBatch) findings.push(...batchFindings)
  }
...
function tokensLine(tokens: TokenUsage): string {
...
export function renderSummary(result: ReviewResult, route: ReviewRoute): string {
  const counts = new Map<ScanSeverity, number>()
  ...
  if (result.findings.length === 0) {
    if (result.unparseableBatches > 0) {
      lines.push(
        'The review produced no parseable findings: every reviewed batch returned text without parseable JSON. '
 ...
 */
export function mapReviewFailure(error: unknown, aborted: boolean): {
  readonly status: 'killed' | 'failed'
  readonly detail: string
  readonly output?: string
} {
  if (aborted) return { status: 'killed', detail: 'scan cancelled' }
