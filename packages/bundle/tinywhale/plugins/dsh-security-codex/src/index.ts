...
import type { JobHooks, JobKindMap, JobOutcome } from '@deepseek-ai/dsh-jobs'
...
export const Config: z<Config> = z.object({
  provider: z.string(),
  model: z.string(),
  effort: z.union(['off', 'high', 'max'] as const).default('off'),
 ...
 */
function resolveRoute(ctx: Context, resolved: ResolvedConfig, agent: Agent): ReviewRoute {
  const header = agent.session.requestHeader()?.config
  const provider = resolved.provider ?? header?.provider
  const model = resolved.model ?? header?.model
  if (provider === undefined || provider.length === 0 || model === undefined || model.length === 0) {
    throw new Error(
      'security_scan cannot resolve a model route: the session has no logged chat request yet, '
  ...
  if (!registered) {
    throw new Error(
      `security_scan provider "${provider}" has no registered LLM adapter. `
...
export function renderOutcome(_args: unknown, value: ScanOutcome): Array<{ type: 'text'; text: string }> {
  return [{
    type: 'text',
...
/** One job's settled outcome gate shared by the async body and cancel. */
interface JobSettle {
  (outcome: JobOutcome): void
}
 ...
 */
function runReviewJob(ctx: Context, options: {
  ...
  let settled = false
  let settle!: JobSettle
  const done = new Promise<JobOutcome>((resolve) => {
    settle = (outcome: JobOutcome) => {
      if (settled) return
      settled = true
      resolve(outcome)
    }
...
export function apply(ctx: Context, config: Config = {}): void {
  ...
  const register = (agent: Agent): void => {
    ...
    const dispose = agent.ctx.tools.register(defineTool({
      ...
      execute: async (args, exec) => {
        ...
        if (agent === undefined) throw new Error('security_scan requires an agent session')
        ...
        if (target === undefined || !isAbsolute(target)) throw new Error('security_scan target must be an absolute path')
        ...
        const id = jobs.start({
          ...
          run: (): JobHooks => runReviewJob(ctx, { resolved, request, route, reportDir }),
        })
  ...
  ctx.effect(() => () => {
    for (const agent of [...disposers.keys()]) disposeAgent(agent)
  })
}
