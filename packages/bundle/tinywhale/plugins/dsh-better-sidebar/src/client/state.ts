...
export function makeDefaultState(
  ...
  if (seedSubagent) {
    leaf.tabs.push({ id: uid('tab'), type: 'subagent', title: t('subagent') })
  }
  if (seedExplorer) {
    leaf.tabs.push({ id: uid('tab'), type: 'explorer', title: t('explorer') })
  }
...
/**
 * Upgrade a still-factory layout (one right-hand leaf whose tabs are only
 * Tasks / Explorer) to the current default pair and order. Custom layouts
 * — splits, extra tab types — stay untouched. Used when a persisted
 * session still has the old single-tab default so a refresh shows both
 * pages instead of only the first.
 */
export function ensureFactoryDefaultTabs(
  state: SidebarState,
  seedExplorer = true,
  seedSubagent = true,
): SidebarState {
  ...
  const wanted: Array<{ type: 'subagent' | 'explorer'; title: string }> = []
  if (seedSubagent) wanted.push({ type: 'subagent', title: t('subagent') })
  if (seedExplorer) wanted.push({ type: 'explorer', title: t('explorer') })
  ...
  const alreadyOrdered = tabs.length === leaf.tabs.length
    && tabs.every((tab, index) => tab.id === leaf.tabs[index]!.id)
  if (alreadyOrdered) return state
  return { ...state, splits: { ...leaf, tabs, active: tabs[0]?.id ?? null } }
}
...
function loadState(sessionId: string, prefs: SidebarPrefs): SidebarState {
  try {
    ...
    if (raw !== null) {
      ...
      if (sanitized !== undefined) {
        return ensureFactoryDefaultTabs(
          sanitized,
          prefs.tabsEnabled['explorer'] !== false,
          prefs.tabsEnabled['subagent'] !== false,
        )
      }
    }
  } catch {
  ...
  // starts open only when the preference says so, and each default tab is
  // skipped when the user disabled that tab type. On a NARROW viewport a
  // brand-new session starts collapsed instead — the panel is a full-screen
  // drawer there, and auto-opening it on first paint would cover the
  // conversation before the user asked. Only the first seeding is affected:
  // once the user expands the drawer, `panelOpen: true` persists like any
  // other state.
