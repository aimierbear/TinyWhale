window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		*/
		...
		/** A fresh default state: Tasks then Explorer in one pane, open per the
		* caller's preference. `width` is the caller's preferred panel width (default
		...
		* `seedSubagent` / `seedExplorer` place those default tabs — the store
		* passes false when the user disabled that tab type in settings, so a
		* fresh session skips a page they turned off instead of opening it. */
		function makeDefaultState(width = 400, panelOpen = true, seedExplorer = true, seedSubagent = true) {
			const leaf = {
				kind: "leaf",
				id: uid("pane"),
				tabs: [],
			...
			if (seedSubagent) leaf.tabs.push({
				id: uid("tab"),
				type: "subagent",
				title: t("subagent")
			});
		...
		function ensureFactoryDefaultTabs(state, seedExplorer = true, seedSubagent = true) {
			if (state.splits.kind !== "leaf") return state;
			const leaf = state.splits;
			if (leaf.tabs.some((tab) => tab.type !== "subagent" && tab.type !== "explorer")) return state;
			const wanted = [];
			if (seedSubagent) wanted.push({
				type: "subagent",
				title: t("subagent")
			});
			if (seedExplorer) wanted.push({
				type: "explorer",
				title: t("explorer")
			});
			const byType = new Map(leaf.tabs.map((tab) => [tab.type, tab]));
			const tabs = wanted.map((item) => byType.get(item.type) ?? {
				id: uid("tab"),
				type: item.type,
				title: item.title
			});
			const alreadyOrdered = tabs.length === leaf.tabs.length && tabs.every((tab, index) => tab.id === leaf.tabs[index].id);
		...
		function loadState(sessionId, prefs) {
			try {
				...
				if (raw !== null) {
					...
					if (sanitized !== void 0) return ensureFactoryDefaultTabs(sanitized, prefs.tabsEnabled["explorer"] !== false, prefs.tabsEnabled["subagent"] !== false);
				}
			} catch {}
			const viewport = typeof window !== "undefined" ? window.innerWidth : void 0;
			return makeDefaultState(viewport === void 0 ? 400 : defaultWidthFor(viewport, prefs.defaultWidthPercent), prefs.openByDefault && (viewport === void 0 || !isNarrowWidth(viewport)), prefs.tabsEnabled["explorer"] !== false, prefs.tabsEnabled["subagent"] !== false);
		}
		/**
		...
		*/
		const SIDEBAR_SERVICE_VERSION = "0.12.0";
		/**
		...
		*/
		...
		//#endregion
		//#region \0dsh-css:/Users/aimierbear/Library/pnpm/store/v11/tmp/_tmp_K01dAl/src/client/sidebar.module.css.mjs
		const css$3 = "._89wInW_toggleCluster{z-index:55;flex-direction:row;gap:4px;display:flex;position:fixed;top:3px;right:10px}._89wInW_panel:not(._89wInW_panelHidden) ._89wInW_tabBar{padding-right:72px}._89wInW_toggleButton{width:28px;height:28px;color:var(--dsw-alias-label-secondary);cursor:pointer;transition:background var(--ds-transition-duration-slow) var(--ds-ease-in-out), color var(--ds-transition-duration-slow) var(--ds-ease-in-out);background:0 0;border:none;border-radius:50%;justify-content:center;align-items:center;display:flex}._89wInW_toggleButton:hover:not(:disabled){background:var(--dsw-alias-interactive-bg-hover);color:var(--dsw-alias-label-primary)}._89wInW_toggleButton:disabled{opacity:.4;cursor:default}._89wInW_panel{z-index:50;background:var(--dsw-specific-sidebar-fill);border-left:1px solid var(--dsw-alias-border-l2);transition:transform var(--ds-transition-duration-slow) var(--ds-ease-in-out), width var(--ds-transition-duration-slow) var(--ds-ease-in-out);flex-direction:column;display:flex;position:fixed;top:0;bottom:0;right:0}._89wInW_panelHidden{pointer-events:none;visibility:hidden;transition:transform var(--ds-transition-duration-slow) var(--ds-ease-in-out), width var(--ds-transition-duration-slow) var(--ds-ease-in-out), visibility 0s linear var(--ds-transition-duration-slow);transform:translate(102%)}._89wInW_panel[data-dragging]{transition:none}._89wInW_panelResize{cursor:col-resize;z-index:2;touch-action:none;width:8px;position:absolute;top:0;bottom:0;left:-4px}._89wInW_panelResizeActive{background:var(--dsw-alias-interactive-bg-hover-accent)}._89wInW_panelBody{flex:1;min-width:0;min-height:0;display:flex}._89wInW_bottomPanel{z-index:50;background:var(--dsw-specific-sidebar-fill);border-top:1px solid var(--dsw-alias-border-l2);transition:transform var(--ds-transition-duration-slow) var(--ds-ease-in-out), height var(--ds-transition-duration-slow) var(--ds-ease-in-out);flex-direction:column;display:flex;position:fixed;bottom:0}._89wInW_bottomPanelHidden{pointer-events:none;visibility:hidden;transition:transform var(--ds-transition-duration-slow) var(--ds-ease-in-out), height var(--ds-transition-duration-slow) var(--ds-ease-in-out), visibility 0s linear var(--ds-transition-duration-slow);transform:translateY(102%)}._89wInW_bottomPanel[data-dragging]{transition:none}._89wInW_bottomResize{cursor:row-resize;z-index:2;touch-action:none;height:8px;position:absolute;top:-4px;left:0;right:0}._89wInW_bottomResizeActive{background:var(--dsw-alias-interactive-bg-hover-accent)}._89wInW_bottomClose{z-index:4;width:28px;height:28px;color:var(--dsw-alias-label-secondary);cursor:pointer;background:0 0;border:none;border-radius:50%;flex:none;justify-content:center;align-items:center;padding:0;display:inline-flex;position:absolute;top:3px;right:6px}._89wInW_bottomClose:hover:not(:disabled){background:var(--dsw-alias-interactive-bg-hover);color:var(--dsw-alias-label-primary)}._89wInW_bottomPanel ._89wInW_tabBar{padding-right:40px}._89wInW_cornerHandle{z-index:52;cursor:nwse-resize;touch-action:none;width:12px;height:12px;position:fixed}._89wInW_cornerHandle:hover,._89wInW_cornerHandle[data-dragging]{background:var(--dsw-alias-interactive-bg-hover-accent)}._89wInW_iconButton{width:28px;height:28px;color:var(--dsw-alias-label-secondary);cursor:pointer;background:0 0;border:none;border-radius:50%;flex:none;justify-content:center;align-items:center;padding:0;display:inline-flex}._89wInW_iconButton:hover:not(:disabled){background:var(--dsw-alias-interactive-bg-hover);color:var(--dsw-alias-label-primary)}._89wInW_iconButton:disabled{opacity:.4;cursor:default}._89wInW_workbench,._89wInW_split{flex:1;min-width:0;min-height:0;display:flex}._89wInW_splitRow{flex-direction:row}._89wInW_splitCol{flex-direction:column}._89wInW_splitChild{display:flex;position:relative;overflow:hidden}._89wInW_divider{z-index:3;touch-action:none;flex:none;position:relative}._89wInW_dividerRow:after,._89wInW_dividerCol:after{content:\"\";background:var(--dsw-alias-border-l2);transition:background var(--ds-transition-duration-slow) var(--ds-ease-in-out);position:absolute}._89wInW_dividerRow{cursor:col-resize;width:7px;margin:0 -2px}._89wInW_dividerRow:after{width:1px;top:0;bottom:0;left:50%;transform:translate(-50%)}._89wInW_dividerCol{cursor:row-resize;height:7px;margin:-2px 0}._89wInW_dividerCol:after{height:1px;top:50%;left:0;right:0;transform:translateY(-50%)}._89wInW_divider:hover:after,._89wInW_dividerActive:after{background:var(--dsw-alias-interactive-bg-hover-accent)}._89wInW_pane{background:var(--dsw-alias-bg-base);flex-direction:column;flex:1;min-width:0;min-height:0;display:flex;position:relative}._89wInW_paneDrop{outline:1px solid var(--dsw-alias-interactive-bg-hover-accent);outline-offset:-1px}._89wInW_dropOverlay{z-index:6;pointer-events:none;background:var(--dsw-alias-interactive-bg-hover-accent);opacity:.5;position:absolute}._89wInW_dropLeft{width:25%;top:0;bottom:0;left:0}._89wInW_dropRight{width:25%;top:0;bottom:0;right:0}._89wInW_dropUp{height:25%;top:0;left:0;right:0}._89wInW_dropDown{height:25%;bottom:0;left:0;right:0}._89wInW_dropCenter{outline:2px dashed var(--dsw-alias-interactive-bg-hover-accent);outline-offset:-2px;background:0 0;inset:25%}._89wInW_paneContent{flex-direction:column;flex:1;min-height:0;display:flex;overflow:hidden}._89wInW_paneTab{flex-direction:column;flex:1;min-height:0;display:flex}._89wInW_paneTabHidden{display:none}._89wInW_paneEmptyCards{flex:1;grid-template-columns:repeat(auto-fill,minmax(140px,1fr));align-content:start;gap:8px;min-height:0;padding:12px;display:grid;overflow:hidden}._89wInW_paneCard{border:1px solid var(--dsw-alias-border-l1);background:var(--dsw-alias-bg-layer-1);min-width:0;color:var(--dsw-alias-label-secondary);font:var(--dsw-font-xxs-strong-12);cursor:pointer;text-align:center;border-radius:8px;flex-direction:column;justify-content:center;align-items:center;gap:6px;padding:12px 8px;display:flex}._89wInW_paneCard:hover:not(:disabled){background:var(--dsw-alias-interactive-bg-hover);color:var(--dsw-alias-label-primary);border-color:var(--dsw-alias-border-l2)}._89wInW_paneCard:disabled{opacity:.45;cursor:default}._89wInW_tabBar{border-bottom:1px solid var(--dsw-alias-border-l1);background:var(--dsw-alias-bg-layer-1);flex:none;align-items:stretch;height:34px;display:flex}._89wInW_tabBarDrop{outline:1px dashed var(--dsw-alias-interactive-bg-hover-accent);outline-offset:-1px}._89wInW_tabList{scrollbar-width:none;flex:1;min-width:0;display:flex;overflow-x:auto}._89wInW_tabList::-webkit-scrollbar{display:none}._89wInW_tab{min-width:64px;max-width:160px;font:var(--dsw-font-xxs-12);color:var(--dsw-alias-label-secondary);border-right:1px solid var(--dsw-alias-border-l1);cursor:pointer;user-select:none;background:0 0;flex:none;align-items:center;gap:4px;padding:0 4px 0 10px;display:flex}._89wInW_tab:hover{background:var(--dsw-alias-interactive-bg-hover)}._89wInW_tabActive{color:var(--dsw-alias-label-primary);background:var(--dsw-alias-interactive-bg-active)}._89wInW_tabTitle{text-overflow:ellipsis;white-space:nowrap;flex:1;min-width:0;overflow:hidden}._89wInW_tabBadge{min-width:16px;height:15px;font:var(--dsw-font-xxxs-strong-11);background:var(--dsw-alias-interactive-bg-hover);color:var(--dsw-alias-brand-primary);border-radius:8px;flex:none;justify-content:center;align-items:center;padding:0 4px;display:inline-flex}._89wInW_tabClose{width:18px;height:18px;color:var(--dsw-alias-label-tertiary);cursor:pointer;background:0 0;border:none;border-radius:4px;flex:none;justify-content:center;align-items:center;padding:0;display:inline-flex}._89wInW_tabClose:hover{background:var(--dsw-alias-interactive-bg-hover);color:var(--dsw-alias-label-primary)}._89wInW_tabBarPlus{background:var(--dsw-alias-bg-layer-1);width:22px;height:22px;color:var(--dsw-alias-label-tertiary);cursor:pointer;border:none;border-radius:5px;flex:none;justify-content:center;align-self:center;align-items:center;margin:0 6px;padding:0;display:inline-flex;position:sticky;right:0}._89wInW_tabBarPlus:hover{background:var(--dsw-alias-interactive-bg-hover);color:var(--dsw-alias-label-primary)}._89wInW_explorer{flex-direction:column;flex:1;min-height:0;display:flex}._89wInW_explorerHeader{flex:none;justify-content:space-between;align-items:center;gap:8px;height:36px;padding:0 8px 0 12px;display:flex}._89wInW_explorerRoot{font:var(--dsw-font-s-14);color:var(--dsw-alias-label-secondary);text-overflow:ellipsis;white-space:nowrap;overflow:hidden}._89wInW_explorerBody{flex:1;min-height:0;padding:2px 6px 8px;overflow-y:auto}._89wInW_explorerRow{width:100%;height:34px;font:var(--dsw-font-s-14);color:var(--dsw-alias-label-primary);text-align:left;cursor:pointer;white-space:nowrap;animation:_89wInW_dsh-row-in .15s var(--ds-ease-in-out);background:0 0;border:none;border-radius:8px;align-items:center;gap:6px;padding:0 8px;display:flex}._89wInW_explorerRow:hover{background:var(--dsw-alias-interactive-bg-hover)}._89wInW_explorerDir{font:var(--dsw-font-s-strong-14)}._89wInW_explorerHidden{opacity:.45}._89wInW_explorerName{text-overflow:ellipsis;overflow:hidden}._89wInW_explorerRef{border:1px solid var(--dsw-alias-border-l1);background:var(--dsw-alias-bg-layer-2);height:20px;color:var(--dsw-alias-label-tertiary);font:var(--dsw-font-xxxs-strong-11);cursor:pointer;border-radius:999px;flex:none;align-items:center;padding:0 8px;display:none}._89wInW_explorerRef:hover{background:var(--dsw-alias-interactive-bg-hover);color:var(--dsw-alias-label-primary)}._89wInW_explorerRow:hover ._89wInW_explorerRef,._89wInW_explorerRow:focus-within ._89wInW_explorerRef{display:inline-flex}._89wInW_explorerCopied{font:var(--dsw-font-xxxs-11);color:var(--dsw-alias-label-tertiary);flex:none}._89wInW_explorerError{font:var(--dsw-font-xxs-12);color:var(--dsw-alias-state-error-primary);cursor:default}@keyframes _89wInW_dsh-row-in{0%{opacity:0}}._89wInW_explorerEmpty{font:var(--dsw-font-xxs-12);color:var(--dsw-alias-label-tertiary);text-align:center;padding:16px}._89wInW_editor{flex-direction:column;flex:1;min-height:0;display:flex}._89wInW_editorHeader{border-bottom:1px solid var(--dsw-alias-border-l1);flex:none;align-items:center;gap:6px;padding:4px 8px;display:flex}._89wInW_editorTitle{min-width:0;font:var(--dsw-font-xxs-strong-12);color:var(--dsw-alias-label-secondary);text-overflow:ellipsis;white-space:nowrap;flex:1;overflow:hidden}._89wInW_editorStatus{font:var(--dsw-font-xxxs-11);color:var(--dsw-alias-label-tertiary)}._89wInW_editorStatusError{color:var(--dsw-alias-state-error-primary)}._89wInW_dirtyDot{background:var(--dsw-alias-state-warn-primary);border-radius:50%;flex:none;width:7px;height:7px}._89wInW_editorPlaceholder{font:var(--dsw-font-xxs-12);color:var(--dsw-alias-label-tertiary);text-align:center;flex:1;justify-content:center;align-items:center;padding:16px;display:flex}._89wInW_orphanedType{opacity:.7;overflow-wrap:anywhere;margin-top:8px;font-size:12px;display:block}._89wInW_editorBinary{text-align:center;flex-direction:column;flex:1;justify-content:center;align-items:center;gap:12px;padding:24px 16px;display:flex}._89wInW_editorBinaryNotice{font:var(--dsw-font-xxs-12);color:var(--dsw-alias-label-tertiary)}._89wInW_editorDownloadLink{border:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-layer-2);color:var(--dsw-alias-label-primary);font:var(--dsw-font-xxs-strong-12);cursor:pointer;transition:background var(--ds-transition-duration-slow) var(--ds-ease-in-out), border-color var(--ds-transition-duration-slow) var(--ds-ease-in-out);border-radius:6px;align-items:center;gap:6px;padding:6px 14px;text-decoration:none;display:inline-flex}._89wInW_editorDownloadLink:hover{background:var(--dsw-alias-interactive-bg-hover);border-color:var(--dsw-alias-border-l2)}._89wInW_editorError{font:var(--dsw-font-xxs-12);color:var(--dsw-alias-state-error-primary);padding:12px 16px}._89wInW_editorBanner{font:var(--dsw-font-xxxs-11);color:var(--dsw-alias-state-warn-label);background:var(--dsw-alias-state-warn-tertiary);flex:none;padding:4px 12px}._89wInW_sandboxStatus{font:var(--dsw-font-xxxs-11);flex:none;align-items:center;gap:8px;padding:4px 10px;display:flex}._89wInW_sandboxStatusOn{color:var(--dsw-alias-label-secondary);background:var(--dsw-alias-bg-layer-1);border-bottom:1px solid var(--dsw-alias-border-l1)}._89wInW_sandboxStatusOff{color:var(--dsw-alias-state-error-primary);background:color-mix(in srgb, var(--dsw-alias-state-error-primary) 10%, transparent);border-bottom:1px solid color-mix(in srgb, var(--dsw-alias-state-error-primary) 45%, transparent)}._89wInW_sandboxDot{background:var(--dsw-alias-state-success-primary);border-radius:50%;flex:none;width:6px;height:6px}._89wInW_sandboxStatusOff ._89wInW_sandboxDot{background:var(--dsw-alias-state-error-primary)}._89wInW_sandboxStatusText{text-overflow:ellipsis;white-space:nowrap;flex:1;min-width:0;overflow:hidden}._89wInW_sandboxAction{border:1px solid var(--dsw-alias-border-l2);font:inherit;color:inherit;cursor:pointer;background:0 0;border-radius:6px;flex:none;padding:2px 8px}._89wInW_sandboxAction:hover{background:var(--dsw-alias-interactive-bg-hover)}._89wInW_editorHtml{background:var(--dsw-alias-bg-base);border:none;flex:1;width:100%;min-height:0}._89wInW_browser{flex-direction:column;flex:1;min-height:0;display:flex}._89wInW_browserBar{border-bottom:1px solid var(--dsw-alias-border-l1);flex:none;align-items:center;gap:4px;padding:6px 8px;display:flex}._89wInW_browserInput{border:1px solid var(--dsw-alias-border-l1);background:var(--dsw-alias-bg-layer-1);min-width:0;height:28px;color:var(--dsw-alias-label-primary);font:var(--dsw-font-xxs-12);border-radius:6px;flex:1;padding:0 10px}._89wInW_browserInput:focus{border-color:var(--dsw-alias-border-l2);outline:none}._89wInW_browserMessage{font:var(--dsw-font-xxxs-11);color:var(--dsw-alias-state-warn-label);background:var(--dsw-alias-state-warn-tertiary);flex:none;padding:4px 12px}._89wInW_browserFrame{background:var(--dsw-alias-bg-base);border:none;flex:1;width:100%;min-height:0}._89wInW_browserStart{text-align:center;min-height:0;font:var(--dsw-font-xs-13);color:var(--dsw-alias-label-tertiary);flex:1;justify-content:center;align-items:center;padding:20px;display:flex}._89wInW_browserBlocked{text-align:center;min-height:0;color:var(--dsw-alias-state-warn-primary);flex-direction:column;flex:1;justify-content:center;align-items:center;gap:6px;padding:24px;display:flex}._89wInW_browserBlockedTitle{font:var(--dsw-font-xxs-strong-12);color:var(--dsw-alias-label-primary)}._89wInW_browserBlockedDesc{max-width:280px;font:var(--dsw-font-xxxs-11);color:var(--dsw-alias-label-secondary)}._89wInW_browserBlockedActions{gap:8px;margin-top:6px;display:flex}._89wInW_browserBlockedButton{border:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-layer-1);color:var(--dsw-alias-label-primary);font:var(--dsw-font-xxxs-11);cursor:pointer;border-radius:6px;padding:4px 12px}._89wInW_browserBlockedButton:hover{background:var(--dsw-alias-interactive-bg-hover)}._89wInW_editorCm{background:0 0;flex:1;min-height:0;overflow:hidden}._89wInW_editorCmHidden{display:none}._89wInW_editorCm .cm-editor{height:100%}._89wInW_editorCm .cm-editor.cm-focused{outline:none}._89wInW_editorModeToggle{border:1px solid var(--dsw-alias-border-l1);background:var(--dsw-alias-bg-layer-1);border-radius:6px;flex:none;align-items:center;gap:2px;padding:2px;display:inline-flex}._89wInW_editorModeButton{color:var(--dsw-alias-label-tertiary);font:var(--dsw-font-xxxs-11);cursor:pointer;background:0 0;border:none;border-radius:4px;padding:2px 8px}._89wInW_editorModeButton:hover{color:var(--dsw-alias-label-primary)}._89wInW_editorModeActive{background:var(--dsw-alias-bg-base);color:var(--dsw-alias-label-primary)}._89wInW_editorImageWrap{flex:1;justify-content:center;align-items:center;min-height:0;padding:12px;display:flex;overflow:auto}._89wInW_editorImage{object-fit:contain;max-width:100%;max-height:100%}._89wInW_editorMd{min-height:0;font:var(--dsw-font-xs-13);flex:1;padding:10px 14px;overflow-y:auto}._89wInW_selectionPopup{z-index:60;border:1px solid var(--dsw-alias-border-l1);background:var(--dsw-alias-bg-layer-2);height:28px;color:var(--dsw-alias-label-primary);font:var(--dsw-font-xxxs-strong-11);white-space:nowrap;cursor:pointer;border-radius:6px;align-items:center;padding:0 10px;display:inline-flex;position:fixed;transform:translate(-50%,calc(-100% - 8px))}._89wInW_selectionPopup:hover{background:var(--dsw-alias-interactive-bg-hover)}._89wInW_editorPdf{background:var(--dsw-alias-bg-base);flex-direction:column;flex:1;min-height:0;display:flex}._89wInW_editorPdfToolbar{border-bottom:1px solid var(--dsw-alias-border-l1);flex:none;justify-content:flex-end;padding:6px 8px;display:flex}._89wInW_editorPdfStage{flex:1;min-height:0;display:flex;position:relative}._89wInW_editorPdfFrame{background:var(--dsw-alias-bg-base);border:none;flex:1;width:100%;min-height:0}._89wInW_editorPdfFrameBlocked{pointer-events:none}._89wInW_editorPdfDragShield{z-index:4;pointer-events:none;background:0 0;position:absolute;inset:0}._89wInW_editorPdfDragShieldActive{pointer-events:auto}body[data-dsh-tab-dragging] ._89wInW_editorPdfFrame{pointer-events:none!important}body[data-dsh-tab-dragging] ._89wInW_editorPdfDragShield{pointer-events:auto!important}._89wInW_terminalWrap{background:var(--dsw-alias-bg-base);flex-direction:column;flex:1;min-height:0;display:flex;position:relative}._89wInW_terminal{flex:1;min-height:0;padding:6px 4px 6px 8px}._89wInW_terminal .xterm{height:100%}._89wInW_terminalBanner{font:var(--dsw-font-xxxs-11);color:var(--dsw-alias-state-warn-label);background:var(--dsw-alias-state-warn-tertiary);flex-wrap:wrap;flex:none;align-items:center;gap:8px;padding:3px 10px;display:flex}._89wInW_terminalBannerUrl{word-break:break-all;opacity:.85;flex-basis:100%;font-family:ui-monospace,SFMono-Regular,Menlo,monospace}._89wInW_boundaryError{z-index:50;background:var(--dsw-alias-bg-layer-1);border-left:1px solid var(--dsw-alias-border-l2);font:var(--dsw-font-xxs-12);color:var(--dsw-alias-state-error-primary);flex-direction:column;align-items:flex-start;gap:8px;padding:16px;display:flex;position:fixed;top:0;bottom:0;right:0;overflow:auto}._89wInW_terminalRetry{border:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-layer-2);color:var(--dsw-alias-label-secondary);font:var(--dsw-font-xxxs-strong-11);cursor:pointer;border-radius:999px;flex:none;padding:1px 8px}._89wInW_terminalRetry:hover{background:var(--dsw-alias-interactive-bg-hover);color:var(--dsw-alias-label-primary)}._89wInW_tabBoundaryError{min-height:0;font:var(--dsw-font-xxs-12);color:var(--dsw-alias-state-error-primary);flex-direction:column;flex:1;align-items:flex-start;gap:8px;padding:12px 16px;display:flex;overflow:auto}._89wInW_git{flex-direction:column;flex:1;min-width:0;min-height:0;display:flex;overflow:hidden auto}._89wInW_gitHeader{flex:none;align-items:center;gap:8px;height:36px;padding:0 8px 0 12px;display:flex}._89wInW_gitBranchSelect{border:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-base);min-width:0;height:26px;color:var(--dsw-alias-label-primary);font:var(--dsw-font-xxs-12);border-radius:6px;flex:1;padding:0 6px}._89wInW_gitSection{border-top:1px solid var(--dsw-alias-border-l1)}._89wInW_gitSectionHeader{font:var(--dsw-font-xxxs-strong-11);color:var(--dsw-alias-label-tertiary);text-transform:uppercase;justify-content:space-between;align-items:center;padding:6px 12px 4px;display:flex}._89wInW_gitLink{font:var(--dsw-font-xxxs-11);color:var(--dsw-alias-brand-primary);cursor:pointer;background:0 0;border:none;padding:0}._89wInW_gitLink:hover:not(:disabled){text-decoration:underline}._89wInW_gitLink:disabled{opacity:.4;cursor:default}._89wInW_gitRow{min-height:34px;animation:_89wInW_dsh-row-in .15s var(--ds-ease-in-out);border-radius:8px;align-items:center;gap:6px;margin:0 6px;padding:0 8px;display:flex}._89wInW_gitRow:hover{background:var(--dsw-alias-interactive-bg-hover)}._89wInW_gitRowSelected{background:var(--dsw-alias-interactive-bg-active)}._89wInW_gitRowMain{cursor:pointer;text-align:left;background:0 0;border:none;flex:1;align-items:center;gap:8px;min-width:0;padding:3px 0;display:flex}._89wInW_gitBadge{width:20px;height:16px;font:var(--dsw-font-xxxs-strong-11);background:var(--dsw-alias-interactive-bg-hover);color:var(--dsw-alias-label-secondary);border-radius:4px;flex:none;justify-content:center;align-items:center;display:inline-flex}._89wInW_gitName{text-overflow:ellipsis;white-space:nowrap;min-width:0;font:var(--dsw-font-s-14);color:var(--dsw-alias-label-primary);flex:1;overflow:hidden}._89wInW_gitEmpty{font:var(--dsw-font-xxs-12);color:var(--dsw-alias-label-tertiary);padding:4px 12px 8px}._89wInW_gitPlaceholder{font:var(--dsw-font-xxs-12);color:var(--dsw-alias-label-tertiary);text-align:center;padding:16px}._89wInW_gitError{font:var(--dsw-font-xxs-12);color:var(--dsw-alias-state-error-primary);white-space:pre-wrap;padding:8px 12px}._89wInW_gitDiff{border-top:1px solid var(--dsw-alias-border-l1);padding:8px}._89wInW_gitDiffTab{flex-direction:column;flex:1;min-width:0;min-height:0;display:flex;overflow:hidden auto}._89wInW_gitDiffTabHeader{border-bottom:1px solid var(--dsw-alias-border-l1);flex:none;align-items:center;gap:8px;height:36px;padding:0 8px 0 12px;display:flex}._89wInW_gitDiffTabTitle{text-overflow:ellipsis;white-space:nowrap;min-width:0;font:var(--dsw-font-xxs-strong-12);color:var(--dsw-alias-label-primary);flex:1;overflow:hidden}._89wInW_gitDiffFile{align-items:baseline;gap:6px;padding:8px 2px 2px;display:flex}._89wInW_gitDiffFilePath{font:var(--dsw-font-xxs-strong-12);color:var(--dsw-alias-label-primary);text-overflow:ellipsis;white-space:nowrap;overflow:hidden}._89wInW_gitDiffFileOld{font:var(--dsw-font-xxxs-11);color:var(--dsw-alias-label-tertiary);text-overflow:ellipsis;white-space:nowrap;flex:none;max-width:40%;overflow:hidden}._89wInW_gitDiffFileTag{border:1px solid var(--dsw-alias-border-l2);font:var(--dsw-font-xxxs-strong-11);color:var(--dsw-alias-label-secondary);border-radius:999px;flex:none;padding:0 6px}._89wInW_gitDiffHunk{font:var(--dsw-font-markdown-code-block-small);color:var(--dsw-alias-label-tertiary);gap:8px;padding:3px 2px;display:flex}._89wInW_gitDiffHunkHeader{color:var(--dsw-alias-label-secondary);flex:none}._89wInW_gitDiffHunkSection{text-overflow:ellipsis;white-space:nowrap;overflow:hidden}._89wInW_gitDiffLine{font:var(--dsw-font-markdown-code-block-small);white-space:pre-wrap;overflow-wrap:anywhere;align-items:stretch;min-width:0;line-height:20px;display:flex}._89wInW_gitDiffNum{text-align:right;width:36px;color:var(--dsw-alias-label-tertiary);user-select:none;flex:none;padding-right:8px}._89wInW_gitDiffCode{flex:1;min-width:0;overflow:visible}._89wInW_gitDiffCtx{color:var(--dsw-alias-label-primary)}._89wInW_gitDiffDel{color:var(--dsw-alias-state-error-primary);background:color-mix(in srgb, var(--dsw-alias-state-error-primary) 12%, transparent)}._89wInW_gitDiffAdd{color:var(--dsw-alias-state-success-primary);background:color-mix(in srgb, var(--dsw-alias-state-success-primary) 12%, transparent)}._89wInW_gitDiffMeta{padding-left:2px}._89wInW_gitDiffMetaText{font:var(--dsw-font-xxxs-11);color:var(--dsw-alias-label-tertiary);font-style:italic}._89wInW_gitDiffExpand{width:100%;font:var(--dsw-font-xxs-12);color:var(--dsw-alias-brand-primary);cursor:pointer;text-align:center;background:0 0;border:none;margin:4px 0;display:block}._89wInW_gitDiffExpand:hover{background:var(--dsw-alias-interactive-bg-hover)}._89wInW_gitConfirmDesc{font:var(--dsw-font-s-14);color:var(--dsw-alias-label-primary);white-space:pre-wrap;margin:0}._89wInW_gitCommit{border-top:1px solid var(--dsw-alias-border-l1);align-items:center;gap:6px;padding:8px 12px;display:flex}._89wInW_gitCommitInput{flex:1;min-width:0}._89wInW_gitCommitButton{background:var(--dsw-alias-button-primary-fill);height:26px;color:var(--dsw-alias-label-primary-inverted);font:var(--dsw-font-xxs-strong-12);cursor:pointer;border:none;border-radius:6px;flex:none;padding:0 12px}._89wInW_gitCommitButton:hover:not(:disabled){background:var(--dsw-alias-button-primary-hover)}._89wInW_gitCommitButton:disabled{opacity:.45;cursor:default}._89wInW_gitLogRow{cursor:pointer;border-radius:8px;flex-direction:column;gap:2px;padding:5px 12px;display:flex}._89wInW_gitLogRow:hover{background:var(--dsw-alias-interactive-bg-hover)}._89wInW_gitLogLine1{align-items:baseline;gap:8px;min-width:0;display:flex}._89wInW_gitLogHash{font:var(--dsw-font-markdown-code-block-small);color:var(--dsw-alias-label-tertiary);flex:none}._89wInW_gitLogLine2{flex-wrap:wrap;align-items:center;gap:6px;min-width:0;display:flex}._89wInW_gitLogRef{border:1px solid var(--dsw-alias-border-l2);font:var(--dsw-font-xxxs-strong-11);color:var(--dsw-alias-brand-primary);white-space:nowrap;border-radius:999px;flex:none;padding:0 5px}._89wInW_gitLogSubject{text-overflow:ellipsis;white-space:nowrap;min-width:0;font:var(--dsw-font-s-14);color:var(--dsw-alias-label-primary);flex:1;overflow:hidden}._89wInW_gitLogMeta{font:var(--dsw-font-xxxs-11);color:var(--dsw-alias-label-tertiary)}._89wInW_gitLogMore{border:1px solid var(--dsw-alias-border-l2);width:calc(100% - 24px);font:var(--dsw-font-xxs-12);color:var(--dsw-alias-label-secondary);cursor:pointer;background:0 0;border-radius:6px;margin:4px 12px 8px;padding:6px 0;display:block}._89wInW_gitLogMore:hover:not(:disabled){background:var(--dsw-alias-interactive-bg-hover);color:var(--dsw-alias-label-primary)}._89wInW_gitLogMore:disabled{opacity:.5;cursor:default}._89wInW_producedRow{flex-wrap:wrap;align-items:center;gap:8px;padding:4px 0;display:flex}._89wInW_producedLabel{font:var(--dsw-font-xxs-12);color:var(--dsw-alias-label-tertiary)}._89wInW_producedChip{border:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-layer-2);max-width:200px;color:var(--dsw-alias-label-secondary);font:var(--dsw-font-xxs-12);cursor:pointer;border-radius:999px;align-items:center;gap:4px;padding:2px 8px;display:inline-flex;overflow:hidden}._89wInW_producedChip:hover{background:var(--dsw-alias-interactive-bg-hover);color:var(--dsw-alias-label-primary)}._89wInW_producedChip span{text-overflow:ellipsis;white-space:nowrap;overflow:hidden}._89wInW_producedMore{font:var(--dsw-font-xxs-12);color:var(--dsw-alias-label-tertiary)}._89wInW_toggleButton:focus-visible,._89wInW_bottomClose:focus-visible,._89wInW_iconButton:focus-visible,._89wInW_tab:focus-visible,._89wInW_tabClose:focus-visible,._89wInW_tabBarPlus:focus-visible,._89wInW_paneCard:focus-visible,._89wInW_explorerRow:focus-visible,._89wInW_explorerRef:focus-visible,._89wInW_gitRowMain:focus-visible,._89wInW_gitLink:focus-visible,._89wInW_gitCommitButton:focus-visible,._89wInW_gitLogRow:focus-visible,._89wInW_gitLogMore:focus-visible,._89wInW_gitDiffExpand:focus-visible,._89wInW_terminalRetry:focus-visible,._89wInW_editorModeButton:focus-visible,._89wInW_editorDownloadLink:focus-visible,._89wInW_editorPptxButton:focus-visible,._89wInW_editorDocxZoomRange:focus-visible{outline:2px solid var(--dsw-alias-interactive-bg-hover-accent);outline-offset:-1px}@media (prefers-reduced-motion:reduce){._89wInW_panel,._89wInW_panelHidden,._89wInW_bottomPanel,._89wInW_bottomPanelHidden,._89wInW_toggleCluster,._89wInW_toggleButton,._89wInW_tab,._89wInW_tabBarPlus,._89wInW_paneCard,._89wInW_explorerRow,._89wInW_gitRow,._89wInW_divider,._89wInW_dividerRow:after,._89wInW_dividerCol:after{transition:none;animation:none}}@media (width<=767px){._89wInW_panel:not(._89wInW_panelHidden) ._89wInW_tabBar{padding-right:40px}._89wInW_tab{min-width:48px;max-width:128px}}";
		...
		var sidebar_module_css_default = {
			"dropUp": "_89wInW_dropUp",
			"editorHeader": "_89wInW_editorHeader",
			"sandboxStatusOff": "_89wInW_sandboxStatusOff",
			"explorerDir": "_89wInW_explorerDir",
			"editorPdfFrameBlocked": "_89wInW_editorPdfFrameBlocked",
			"gitBadge": "_89wInW_gitBadge",
			"panelBody": "_89wInW_panelBody",
			"gitError": "_89wInW_gitError",
			"dirtyDot": "_89wInW_dirtyDot",
			"gitDiffCode": "_89wInW_gitDiffCode",
			"bottomResize": "_89wInW_bottomResize",
			"tabBarDrop": "_89wInW_tabBarDrop",
			"workbench": "_89wInW_workbench",
			"toggleButton": "_89wInW_toggleButton",
			"gitLogLine2": "_89wInW_gitLogLine2",
			"paneTab": "_89wInW_paneTab",
			"panelResizeActive": "_89wInW_panelResizeActive",
			"tabBadge": "_89wInW_tabBadge",
			"dropOverlay": "_89wInW_dropOverlay",
			"paneTabHidden": "_89wInW_paneTabHidden",
			"explorerRef": "_89wInW_explorerRef",
			"editorTitle": "_89wInW_editorTitle",
			"boundaryError": "_89wInW_boundaryError",
			"gitDiffTab": "_89wInW_gitDiffTab",
			"explorerHidden": "_89wInW_explorerHidden",
			"paneDrop": "_89wInW_paneDrop",
			"cornerHandle": "_89wInW_cornerHandle",
			"tab": "_89wInW_tab",
			"editorError": "_89wInW_editorError",
			"editorStatusError": "_89wInW_editorStatusError",
			"dsh-row-in": "_89wInW_dsh-row-in",
			"gitPlaceholder": "_89wInW_gitPlaceholder",
			"gitLogHash": "_89wInW_gitLogHash",
			"gitLogRef": "_89wInW_gitLogRef",
			"sandboxStatus": "_89wInW_sandboxStatus",
			"browserBlockedButton": "_89wInW_browserBlockedButton",
			"editorBanner": "_89wInW_editorBanner",
			"editorPdfDragShield": "_89wInW_editorPdfDragShield",
			"explorerRow": "_89wInW_explorerRow",
			"editorBinaryNotice": "_89wInW_editorBinaryNotice",
			"editorPdfToolbar": "_89wInW_editorPdfToolbar",
			"gitDiffCtx": "_89wInW_gitDiffCtx",
			"gitLogMore": "_89wInW_gitLogMore",
			"panelResize": "_89wInW_panelResize",
			"sandboxDot": "_89wInW_sandboxDot",
			"dropRight": "_89wInW_dropRight",
			"gitDiffNum": "_89wInW_gitDiffNum",
			"splitChild": "_89wInW_splitChild",
			"tabList": "_89wInW_tabList",
			"editorPlaceholder": "_89wInW_editorPlaceholder",
			"paneEmptyCards": "_89wInW_paneEmptyCards",
			"browser": "_89wInW_browser",
			"editorCmHidden": "_89wInW_editorCmHidden",
			"terminalBanner": "_89wInW_terminalBanner",
			"gitHeader": "_89wInW_gitHeader",
			"gitDiffFileTag": "_89wInW_gitDiffFileTag",
			"browserMessage": "_89wInW_browserMessage",
			"editorPdf": "_89wInW_editorPdf",
			"gitDiffFile": "_89wInW_gitDiffFile",
			"explorerName": "_89wInW_explorerName",
			"terminal": "_89wInW_terminal",
			"editorStatus": "_89wInW_editorStatus",
			"gitDiffTabHeader": "_89wInW_gitDiffTabHeader",
			"gitDiffHunkHeader": "_89wInW_gitDiffHunkHeader",
			"dropDown": "_89wInW_dropDown",
			"gitCommitButton": "_89wInW_gitCommitButton",
			"tabActive": "_89wInW_tabActive",
			"producedMore": "_89wInW_producedMore",
			"panelHidden": "_89wInW_panelHidden",
			"dividerRow": "_89wInW_dividerRow",
			"sandboxAction": "_89wInW_sandboxAction",
			"terminalWrap": "_89wInW_terminalWrap",
			"gitDiff": "_89wInW_gitDiff",
			"paneCard": "_89wInW_paneCard",
			"explorerBody": "_89wInW_explorerBody",
			"gitRow": "_89wInW_gitRow",
			"explorerHeader": "_89wInW_explorerHeader",
			"gitDiffFileOld": "_89wInW_gitDiffFileOld",
			"editorCm": "_89wInW_editorCm",
			"gitDiffHunk": "_89wInW_gitDiffHunk",
			"toggleCluster": "_89wInW_toggleCluster",
			"gitLogRow": "_89wInW_gitLogRow",
			"producedChip": "_89wInW_producedChip",
			"gitEmpty": "_89wInW_gitEmpty",
			"tabBarPlus": "_89wInW_tabBarPlus",
			"git": "_89wInW_git",
			"bottomPanelHidden": "_89wInW_bottomPanelHidden",
			"pane": "_89wInW_pane",
			"gitName": "_89wInW_gitName",
			"gitDiffExpand": "_89wInW_gitDiffExpand",
			"gitLogMeta": "_89wInW_gitLogMeta",
			"splitRow": "_89wInW_splitRow",
			"editorPptxButton": "_89wInW_editorPptxButton",
			"browserBar": "_89wInW_browserBar",
			"editorModeButton": "_89wInW_editorModeButton",
			"dropLeft": "_89wInW_dropLeft",
			"editorMd": "_89wInW_editorMd",
			"selectionPopup": "_89wInW_selectionPopup",
			"gitDiffMetaText": "_89wInW_gitDiffMetaText",
			"explorerError": "_89wInW_explorerError",
			"gitDiffHunkSection": "_89wInW_gitDiffHunkSection",
			"tabTitle": "_89wInW_tabTitle",
			"editorImageWrap": "_89wInW_editorImageWrap",
			"paneContent": "_89wInW_paneContent",
			"gitLogLine1": "_89wInW_gitLogLine1",
			"editor": "_89wInW_editor",
			"sandboxStatusText": "_89wInW_sandboxStatusText",
			"browserBlockedDesc": "_89wInW_browserBlockedDesc",
			"dividerCol": "_89wInW_dividerCol",
			"gitDiffFilePath": "_89wInW_gitDiffFilePath",
			"terminalRetry": "_89wInW_terminalRetry",
			"editorModeActive": "_89wInW_editorModeActive",
			"gitCommit": "_89wInW_gitCommit",
			"panel": "_89wInW_panel",
			"tabBoundaryError": "_89wInW_tabBoundaryError",
			"explorerCopied": "_89wInW_explorerCopied",
			"dropCenter": "_89wInW_dropCenter",
			"editorDownloadLink": "_89wInW_editorDownloadLink",
			"browserFrame": "_89wInW_browserFrame",
			"gitSectionHeader": "_89wInW_gitSectionHeader",
			"divider": "_89wInW_divider",
			"explorerEmpty": "_89wInW_explorerEmpty",
			"bottomPanel": "_89wInW_bottomPanel",
			"gitBranchSelect": "_89wInW_gitBranchSelect",
			"explorerRoot": "_89wInW_explorerRoot",
			"browserBlocked": "_89wInW_browserBlocked",
			"gitRowSelected": "_89wInW_gitRowSelected",
			"sandboxStatusOn": "_89wInW_sandboxStatusOn",
			"gitCommitInput": "_89wInW_gitCommitInput",
			"browserBlockedTitle": "_89wInW_browserBlockedTitle",
			"terminalBannerUrl": "_89wInW_terminalBannerUrl",
			"bottomResizeActive": "_89wInW_bottomResizeActive",
			"editorDocxZoomRange": "_89wInW_editorDocxZoomRange",
			"gitSection": "_89wInW_gitSection",
			"producedLabel": "_89wInW_producedLabel",
			"gitLink": "_89wInW_gitLink",
			"gitRowMain": "_89wInW_gitRowMain",
			"iconButton": "_89wInW_iconButton",
			"gitDiffTabTitle": "_89wInW_gitDiffTabTitle",
			"producedRow": "_89wInW_producedRow",
			"tabClose": "_89wInW_tabClose",
			"gitLogSubject": "_89wInW_gitLogSubject",
			"editorPdfFrame": "_89wInW_editorPdfFrame",
			"gitDiffLine": "_89wInW_gitDiffLine",
			"gitDiffMeta": "_89wInW_gitDiffMeta",
			"gitConfirmDesc": "_89wInW_gitConfirmDesc",
			"browserInput": "_89wInW_browserInput",
			"browserBlockedActions": "_89wInW_browserBlockedActions",
			"gitDiffAdd": "_89wInW_gitDiffAdd",
			"splitCol": "_89wInW_splitCol",
			"orphanedType": "_89wInW_orphanedType",
			"editorPdfDragShieldActive": "_89wInW_editorPdfDragShieldActive",
			"browserStart": "_89wInW_browserStart",
			"tabBar": "_89wInW_tabBar",
			"editorPdfStage": "_89wInW_editorPdfStage",
			"bottomClose": "_89wInW_bottomClose",
			"split": "_89wInW_split",
			"explorer": "_89wInW_explorer",
			"gitDiffDel": "_89wInW_gitDiffDel",
			"editorModeToggle": "_89wInW_editorModeToggle",
			"editorBinary": "_89wInW_editorBinary",
			"editorImage": "_89wInW_editorImage",
			"dividerActive": "_89wInW_dividerActive",
			"editorHtml": "_89wInW_editorHtml"
		};
		//#endregion
		...
		*/
		...
		//#endregion
		//#region \0dsh-css:/Users/aimierbear/Library/pnpm/store/v11/tmp/_tmp_K01dAl/src/client/SubagentView.module.css.mjs
		const css$2 = ".HFZcja_subagent{flex-direction:column;flex:1;min-height:0;display:flex}.HFZcja_subagentHeader{flex:none;align-items:center;gap:8px;height:36px;padding:0 8px 0 12px;display:flex}.HFZcja_subagentTitle{min-width:0;font:var(--dsw-font-s-14);color:var(--dsw-alias-label-secondary);text-overflow:ellipsis;white-space:nowrap;flex:1;overflow:hidden}.HFZcja_subagentCount{font:var(--dsw-font-xxxs-11);color:var(--dsw-alias-label-tertiary);flex:none}.HFZcja_subagentRefresh{width:24px;height:24px;color:var(--dsw-alias-label-secondary);cursor:pointer;background:0 0;border:none;border-radius:6px;flex:none;justify-content:center;align-items:center;display:inline-flex}.HFZcja_subagentRefresh:hover{background:var(--dsw-alias-interactive-bg-hover)}.HFZcja_subagentBody{flex:1;min-height:0;padding:2px 6px 8px;overflow-y:auto}.HFZcja_subagentRow{box-sizing:border-box;width:100%;min-height:50px;font:var(--dsw-font-s-14);color:var(--dsw-alias-label-primary);text-align:left;cursor:pointer;background:0 0;border:none;border-radius:8px;outline:none;align-items:flex-start;gap:8px;padding:7px 8px 7px 11px;display:flex;position:relative}.HFZcja_subagentRow:hover,.HFZcja_subagentRow:focus-visible{background:var(--dsw-alias-interactive-bg-hover)}.HFZcja_subagentRowActive,.HFZcja_subagentRowActive:hover,.HFZcja_subagentRowActive:focus-visible{background:var(--dsw-alias-interactive-bg-active)}.HFZcja_subagentRowDisabled{color:var(--dsw-alias-label-dimmed);cursor:not-allowed}.HFZcja_subagentRowDisabled:hover{background:0 0}.HFZcja_subagentRowLoading{cursor:default}.HFZcja_subagentDot{margin-top:4px}.HFZcja_subagentContent{flex-direction:column;flex:1;gap:2px;min-width:0;display:flex}.HFZcja_subagentLabel,.HFZcja_subagentSecondary{text-overflow:ellipsis;white-space:nowrap;overflow:hidden}.HFZcja_subagentLabel{color:inherit;font-weight:400}.HFZcja_subagentSecondary{font:var(--dsw-font-xxxs-11);color:var(--dsw-alias-label-tertiary)}.HFZcja_subagentLive{min-width:0;font:var(--dsw-font-xxxs-11);color:var(--dsw-alias-label-tertiary);align-items:baseline;gap:4px;display:flex;overflow:hidden}.HFZcja_subagentLiveTool{font:var(--dsw-font-xxxs-strong-11);color:var(--dsw-alias-label-secondary);flex:none}.HFZcja_subagentLiveArgs{min-width:0;font-family:var(--ds-font-family-code);font-size:var(--dsw-font-xxxs-11-font-size);line-height:var(--dsw-font-xxxs-11-line-height);color:var(--dsw-alias-label-tertiary);text-overflow:ellipsis;white-space:nowrap;overflow:hidden}.HFZcja_subagentLiveText{-webkit-line-clamp:2;font:var(--dsw-font-xxxs-11);color:var(--dsw-alias-label-secondary);-webkit-box-orient:vertical;display:-webkit-box;overflow:hidden}.HFZcja_subagentNode{min-width:0;position:relative}.HFZcja_subagentChildren{margin-left:18px;padding-left:4px;position:relative}.HFZcja_subagentChildren:before{content:\"\";border-left:1px solid var(--dsw-alias-border-l2);height:26px;position:absolute;top:-26px;left:0}.HFZcja_subagentChildren[aria-busy=true]:before{content:none}.HFZcja_subagentChildren>.HFZcja_subagentNode:before{content:\"\";border-left:1px solid var(--dsw-alias-border-l2);position:absolute;top:0;bottom:0;left:-4px}.HFZcja_subagentChildren>.HFZcja_subagentNode:last-child:before{height:17px;bottom:auto}.HFZcja_subagentChildren>.HFZcja_subagentNode>.HFZcja_subagentRow:before{content:\"\";border-top:1px solid var(--dsw-alias-border-l2);width:14px;position:absolute;top:16px;left:-4px}.HFZcja_subagentEmpty{font:var(--dsw-font-xxs-12);color:var(--dsw-alias-label-tertiary);text-align:center;flex-direction:column;gap:2px;padding:16px;display:flex}.HFZcja_subagentEmptyHint{font:var(--dsw-font-xxxs-11);color:var(--dsw-alias-label-dimmed)}.HFZcja_subagentError{font:var(--dsw-font-xxs-12);color:var(--dsw-alias-state-error-primary);justify-content:space-between;align-items:center;gap:8px;padding:8px 10px;display:flex}.HFZcja_subagentErrorRetry{height:24px;color:var(--dsw-alias-label-secondary);font:var(--dsw-font-xxxs-strong-11);cursor:pointer;background:0 0;border:none;border-radius:6px;flex:none;align-items:center;gap:4px;padding:0 8px;display:inline-flex}.HFZcja_subagentErrorRetry:hover{background:var(--dsw-alias-interactive-bg-hover);color:var(--dsw-alias-label-primary)}.HFZcja_jobs{border-top:1px solid var(--dsw-alias-border-l2);margin-top:10px;padding-top:8px}.HFZcja_jobsHeader{align-items:center;gap:8px;height:26px;padding:0 2px;display:flex}.HFZcja_jobsTitle{min-width:0;font:var(--dsw-font-xxxs-strong-11);color:var(--dsw-alias-label-secondary);text-overflow:ellipsis;white-space:nowrap;flex:1;overflow:hidden}.HFZcja_jobsCount{font:var(--dsw-font-xxxs-11);color:var(--dsw-alias-label-tertiary);flex:none}.HFZcja_jobsList{flex-direction:column;gap:2px;margin:0;padding:0;list-style:none;display:flex}.HFZcja_jobsRow{border-radius:8px;align-items:center;gap:4px;display:flex}.HFZcja_jobsRow:hover{background:var(--dsw-alias-interactive-bg-hover)}.HFZcja_jobsRowSettled{opacity:.8}.HFZcja_jobsRowSelected,.HFZcja_jobsRowSelected:hover{background:var(--dsw-alias-interactive-bg-active)}.HFZcja_jobsRowMain{min-width:0;font:var(--dsw-font-s-14);color:var(--dsw-alias-label-primary);text-align:left;cursor:pointer;background:0 0;border:none;border-radius:8px;outline:none;flex:1;align-items:flex-start;gap:8px;padding:6px 8px 6px 11px;display:flex}.HFZcja_jobsRowMain:focus-visible{background:var(--dsw-alias-interactive-bg-hover)}.HFZcja_jobsDot{margin-top:5px}.HFZcja_jobsContent{flex-direction:column;gap:1px;min-width:0;display:flex}.HFZcja_jobsLabelLine{align-items:center;gap:6px;min-width:0;display:flex}.HFZcja_jobsKind{text-overflow:ellipsis;white-space:nowrap;border:1px solid var(--dsw-alias-border-l2);max-width:90px;font:var(--dsw-font-xxxs-strong-11);color:var(--dsw-alias-label-tertiary);border-radius:4px;flex:none;padding:0 5px;line-height:14px;overflow:hidden}.HFZcja_jobsLabel{text-overflow:ellipsis;white-space:nowrap;min-width:0;font-family:var(--ds-font-family-code);font-size:var(--dsw-font-xxxs-11-font-size);line-height:var(--dsw-font-xxxs-11-line-height);color:var(--dsw-alias-label-primary);flex:1;overflow:hidden}.HFZcja_jobsSecondary{text-overflow:ellipsis;white-space:nowrap;font:var(--dsw-font-xxxs-11);color:var(--dsw-alias-label-tertiary);overflow:hidden}.HFZcja_jobsKill{width:22px;height:22px;color:var(--dsw-alias-label-secondary);cursor:pointer;background:0 0;border:none;border-radius:6px;flex:none;justify-content:center;align-items:center;margin-right:4px;display:inline-flex}.HFZcja_jobsKill:hover{background:color-mix(in srgb, var(--dsw-alias-state-error-primary) 12%, transparent);color:var(--dsw-alias-state-error-primary)}.HFZcja_jobsKillArmed,.HFZcja_jobsKillArmed:hover{background:color-mix(in srgb, var(--dsw-alias-state-error-primary) 12%, transparent);width:auto;height:20px;color:var(--dsw-alias-state-error-primary);font:var(--dsw-font-xxxs-strong-11);white-space:nowrap;padding:0 8px}.HFZcja_jobsKill:disabled{opacity:.5;cursor:default}.HFZcja_jobsKillError{font:var(--dsw-font-xxxs-11);color:var(--dsw-alias-state-error-primary);flex:none;margin-right:4px}.HFZcja_jobsPane{z-index:1;border:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-base);border-radius:8px;margin-top:4px;position:sticky;bottom:0;overflow:hidden;box-shadow:0 -6px 12px -8px #00000059}.HFZcja_jobsPaneHeader{border-bottom:1px solid var(--dsw-alias-border-l1);align-items:center;gap:6px;height:28px;padding:0 4px 0 10px;display:flex}.HFZcja_jobsPaneDot{flex:none}.HFZcja_jobsPaneLabel{text-overflow:ellipsis;white-space:nowrap;min-width:0;font-family:var(--ds-font-family-code);font-size:var(--dsw-font-xxxs-11-font-size);line-height:var(--dsw-font-xxxs-11-line-height);color:var(--dsw-alias-label-primary);flex:1;overflow:hidden}.HFZcja_jobsPaneStatus{font:var(--dsw-font-xxxs-11);color:var(--dsw-alias-label-tertiary);flex:none}.HFZcja_jobsPaneClose{width:20px;height:20px;color:var(--dsw-alias-label-secondary);cursor:pointer;background:0 0;border:none;border-radius:5px;flex:none;justify-content:center;align-items:center;display:inline-flex}.HFZcja_jobsPaneClose:hover{background:var(--dsw-alias-interactive-bg-hover)}.HFZcja_jobsPanePre{max-height:200px;font-family:var(--ds-font-family-code);font-size:var(--dsw-font-xxxs-11-font-size);color:var(--dsw-alias-label-primary);white-space:pre-wrap;word-break:break-word;margin:0;padding:6px 10px;line-height:1.5;overflow:auto}.HFZcja_jobsPaneHint{font:var(--dsw-font-xxxs-11);color:var(--dsw-alias-label-tertiary);padding:8px 10px}.HFZcja_jobsPaneError{color:var(--dsw-alias-state-error-primary)}";
		...
		var SubagentView_module_css_default = {
			"subagentLabel": "HFZcja_subagentLabel",
			"subagentLiveText": "HFZcja_subagentLiveText",
			"jobsTitle": "HFZcja_jobsTitle",
			"subagentRowLoading": "HFZcja_subagentRowLoading",
			"jobsContent": "HFZcja_jobsContent",
			"subagentErrorRetry": "HFZcja_subagentErrorRetry",
			"subagentCount": "HFZcja_subagentCount",
			"jobs": "HFZcja_jobs",
			"subagentRow": "HFZcja_subagentRow",
			"jobsDot": "HFZcja_jobsDot",
			"jobsKind": "HFZcja_jobsKind",
			"jobsLabel": "HFZcja_jobsLabel",
			"subagentContent": "HFZcja_subagentContent",
			"jobsSecondary": "HFZcja_jobsSecondary",
			"jobsKill": "HFZcja_jobsKill",
			"jobsKillArmed": "HFZcja_jobsKillArmed",
			"subagentDot": "HFZcja_subagentDot",
			"subagentTitle": "HFZcja_subagentTitle",
			"subagentRowActive": "HFZcja_subagentRowActive",
			"jobsPaneHint": "HFZcja_jobsPaneHint",
			"jobsPaneError": "HFZcja_jobsPaneError",
			"jobsCount": "HFZcja_jobsCount",
			"subagentSecondary": "HFZcja_subagentSecondary",
			"jobsRowMain": "HFZcja_jobsRowMain",
			"subagentRefresh": "HFZcja_subagentRefresh",
			"jobsHeader": "HFZcja_jobsHeader",
			"jobsPaneDot": "HFZcja_jobsPaneDot",
			"subagentBody": "HFZcja_subagentBody",
			"subagentNode": "HFZcja_subagentNode",
			"subagentLive": "HFZcja_subagentLive",
			"subagentEmptyHint": "HFZcja_subagentEmptyHint",
			"jobsLabelLine": "HFZcja_jobsLabelLine",
			"jobsPane": "HFZcja_jobsPane",
			"jobsList": "HFZcja_jobsList",
			"subagentLiveTool": "HFZcja_subagentLiveTool",
			"jobsPaneLabel": "HFZcja_jobsPaneLabel",
			"jobsPaneStatus": "HFZcja_jobsPaneStatus",
			"subagentHeader": "HFZcja_subagentHeader",
			"jobsPaneClose": "HFZcja_jobsPaneClose",
			"jobsPanePre": "HFZcja_jobsPanePre",
			"jobsRowSettled": "HFZcja_jobsRowSettled",
			"subagentError": "HFZcja_subagentError",
			"jobsKillError": "HFZcja_jobsKillError",
			"jobsPaneHeader": "HFZcja_jobsPaneHeader",
			"subagent": "HFZcja_subagent",
			"subagentLiveArgs": "HFZcja_subagentLiveArgs",
			"subagentChildren": "HFZcja_subagentChildren",
			"subagentEmpty": "HFZcja_subagentEmpty",
			"subagentRowDisabled": "HFZcja_subagentRowDisabled",
			"jobsRow": "HFZcja_jobsRow",
			"jobsRowSelected": "HFZcja_jobsRowSelected"
		};
		//#endregion
		...
		*/
		...
		//#endregion
		//#region \0dsh-css:/Users/aimierbear/Library/pnpm/store/v11/tmp/_tmp_K01dAl/src/client/SideCardSection.module.css.mjs
		const css$1 = ".L-z1Va_section{flex-direction:column;gap:14px;width:100%;height:100%;min-height:0;display:flex;overflow-y:auto}.L-z1Va_intro{color:var(--dsw-alias-label-tertiary);margin:0;padding:0 2px;font-size:13px;line-height:20px}.L-z1Va_group{box-sizing:border-box;border:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-layer-3);border-radius:16px;flex-direction:column;flex:none;gap:8px;padding:18px 20px 20px;display:flex}.L-z1Va_groupHeading{color:var(--dsw-alias-label-primary);align-items:baseline;gap:7px;padding:0 2px 6px;font-size:13px;font-weight:600;line-height:20px;display:flex}.L-z1Va_count{color:var(--dsw-alias-label-tertiary);font-variant-numeric:tabular-nums;font-size:12px;font-weight:400;line-height:18px}.L-z1Va_grid{grid-template-columns:repeat(auto-fill,minmax(168px,1fr));gap:10px;display:grid}.L-z1Va_card{border:1px solid var(--dsw-alias-border-l2);font:inherit;color:inherit;cursor:pointer;background:0 0;border-radius:12px;flex-direction:column;transition:background .12s,border-color .12s;display:flex;position:relative}.L-z1Va_card:not(.L-z1Va_cardOn):hover{background:var(--dsw-alias-interactive-bg-hover);border-color:var(--dsw-alias-label-dimmed)}.L-z1Va_cardOn{border-color:var(--dsw-alias-button-primary-fill);background:var(--dsw-alias-interactive-bg-active)}.L-z1Va_cardMain{border-radius:inherit;width:100%;font:inherit;color:inherit;text-align:left;cursor:pointer;background:0 0;border:0;flex-direction:column;gap:6px;padding:12px;display:flex}.L-z1Va_cardMain:focus-visible,.L-z1Va_cardGear:focus-visible{outline:2px solid var(--dsw-alias-border-l4);outline-offset:2px}.L-z1Va_cardTop{align-items:center;gap:8px;min-width:0;display:flex}.L-z1Va_cardIconChip{border:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-layer-2);width:28px;height:28px;color:var(--dsw-alias-label-tertiary);border-radius:8px;flex:none;justify-content:center;align-items:center;display:inline-flex}.L-z1Va_cardOn .L-z1Va_cardIconChip{border-color:color-mix(in srgb, var(--dsw-alias-button-primary-fill) 35%, transparent);background:color-mix(in srgb, var(--dsw-alias-button-primary-fill) 12%, transparent);color:var(--dsw-alias-button-primary-fill)}.L-z1Va_cardTitle{min-width:0;color:var(--dsw-alias-label-secondary);white-space:nowrap;text-overflow:ellipsis;flex:1;font-size:13px;font-weight:600;line-height:20px;overflow:hidden}.L-z1Va_cardOn .L-z1Va_cardTitle{color:var(--dsw-alias-label-primary)}.L-z1Va_cardCheck{background:var(--dsw-alias-button-primary-fill);width:16px;height:16px;color:var(--dsw-alias-bg-layer-3);border-radius:50%;flex:none;justify-content:center;align-items:center;display:inline-flex}.L-z1Va_cardDesc{color:var(--dsw-alias-label-tertiary);white-space:nowrap;text-overflow:ellipsis;font-size:11px;line-height:16px;overflow:hidden}.L-z1Va_addCard{border-style:dashed;border-color:var(--dsw-alias-border-l2);text-align:left;align-items:flex-start;padding:12px}.L-z1Va_addCard:hover{background:var(--dsw-alias-interactive-bg-hover);border-color:var(--dsw-alias-interactive-bg-hover-accent);color:var(--dsw-alias-label-primary)}.L-z1Va_addCard:hover .L-z1Va_cardTitle{color:var(--dsw-alias-label-primary)}.L-z1Va_addCard:hover .L-z1Va_cardIconChip{border-color:color-mix(in srgb, var(--dsw-alias-button-primary-fill) 35%, transparent);color:var(--dsw-alias-button-primary-fill)}.L-z1Va_addCard:focus-visible{outline:2px solid var(--dsw-alias-border-l4);outline-offset:2px}.L-z1Va_cardOn .L-z1Va_cardDesc{color:var(--dsw-alias-label-secondary)}.L-z1Va_cardWithGear .L-z1Va_cardDesc{padding-right:30px}.L-z1Va_cardGear{width:16px;height:16px;color:var(--dsw-alias-label-tertiary);cursor:pointer;background:0 0;border:0;border-radius:50%;justify-content:center;align-items:center;padding:0;display:inline-flex;position:absolute;top:46px;right:12px}.L-z1Va_cardGear:hover{color:var(--dsw-alias-label-primary);background:var(--dsw-alias-interactive-bg-hover)}.L-z1Va_row{border-bottom:1px solid var(--dsw-alias-border-l2);justify-content:space-between;align-items:center;gap:16px;padding:12px 2px;display:flex}.L-z1Va_row:last-child{border-bottom:none}.L-z1Va_rowText{flex-direction:column;gap:4px;min-width:0;display:flex}.L-z1Va_title{color:var(--dsw-alias-label-primary);font-size:14px;line-height:22px}.L-z1Va_desc{color:var(--dsw-alias-label-tertiary);font-size:12px;line-height:18px}.L-z1Va_switch{cursor:pointer;flex:none;display:inline-flex;position:relative}.L-z1Va_switchInput{opacity:0;width:1px;height:1px;margin:0;position:absolute}.L-z1Va_switchTrack{box-sizing:border-box;border:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-layer-1);border-radius:10px;align-items:center;width:36px;height:20px;padding:2px;transition:background .15s,border-color .15s;display:inline-flex}.L-z1Va_switchThumb{background:var(--dsw-alias-label-secondary);border-radius:50%;width:14px;height:14px;transition:transform .15s,background .15s;display:block}.L-z1Va_switch:hover .L-z1Va_switchTrack{border-color:var(--dsw-alias-label-dimmed)}.L-z1Va_switchInput:checked+.L-z1Va_switchTrack{border-color:var(--dsw-alias-button-primary-fill);background:var(--dsw-alias-button-primary-fill)}.L-z1Va_switchInput:checked+.L-z1Va_switchTrack .L-z1Va_switchThumb{background:var(--dsw-alias-bg-layer-3);transform:translate(16px)}.L-z1Va_switchInput:focus-visible+.L-z1Va_switchTrack{outline:2px solid var(--dsw-alias-state-business-primary);outline-offset:2px}.L-z1Va_control{flex:none;align-items:center;gap:6px;display:flex}.L-z1Va_percentInput{width:76px}.L-z1Va_typedInput{width:200px}.L-z1Va_typedInputNumber{width:76px}.L-z1Va_suffix{color:var(--dsw-alias-label-secondary);font-size:14px;line-height:22px}.L-z1Va_popupDialog.L-z1Va_popupDialog{width:min(460px,100%)}.L-z1Va_popupRows{flex-direction:column;width:100%;display:flex}.L-z1Va_popupRow{border-bottom:1px solid var(--dsw-alias-border-l2);justify-content:space-between;align-items:center;gap:16px;padding:14px 2px;display:flex}.L-z1Va_popupRow:hover{background:var(--dsw-alias-interactive-bg-hover)}.L-z1Va_popupRows>:last-child{border-bottom:none}.L-z1Va_done{appearance:none;font:inherit;cursor:pointer;background:var(--dsw-alias-label-primary);color:var(--dsw-alias-bg-layer-3);border:1px solid #0000;border-radius:8px;padding:5px 14px;font-size:13px;line-height:1.5}.L-z1Va_done:focus-visible{outline:2px solid var(--dsw-alias-brand-primary);outline-offset:1px}.L-z1Va_error{color:var(--dsw-alias-state-error-primary);padding:10px 0 2px;font-size:12px;line-height:17px}.L-z1Va_pluginModal.L-z1Va_pluginModal{width:min(560px,100%)}.L-z1Va_pluginList{flex-direction:column;gap:12px;width:100%;display:flex}.L-z1Va_pluginTopicBtn{appearance:none;border:1px solid var(--dsw-alias-border-l2);width:100%;font:inherit;color:var(--dsw-alias-label-secondary);background:var(--dsw-alias-bg-layer-1);cursor:pointer;border-radius:8px;padding:6px 12px;font-size:12px;line-height:18px}.L-z1Va_pluginTopicBtn:hover{background:var(--dsw-alias-interactive-bg-hover);border-color:var(--dsw-alias-interactive-bg-hover-accent);color:var(--dsw-alias-label-primary)}.L-z1Va_pluginTopicBtn:focus-visible{outline:2px solid var(--dsw-alias-border-l4);outline-offset:1px}.L-z1Va_pluginEmpty{color:var(--dsw-alias-label-tertiary);padding:20px 2px;font-size:12px;line-height:18px}.L-z1Va_pluginEntries{flex-direction:column;gap:10px;display:flex}.L-z1Va_pluginEntry{border:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-layer-1);border-radius:12px;flex-direction:column;gap:4px;padding:12px;display:flex}.L-z1Va_pluginEntryHead{justify-content:space-between;align-items:center;gap:12px;display:flex}.L-z1Va_pluginEntryActions{flex:none;align-items:center;gap:6px;display:inline-flex}.L-z1Va_pluginJumpBtn{appearance:none;border:1px solid var(--dsw-alias-border-l2);font:inherit;cursor:pointer;color:var(--dsw-alias-label-secondary);background:0 0;border-radius:8px;flex:none;padding:3px 12px;font-size:12px;line-height:1.5}.L-z1Va_pluginJumpBtn:hover{background:var(--dsw-alias-interactive-bg-hover);border-color:var(--dsw-alias-interactive-bg-hover-accent);color:var(--dsw-alias-label-primary)}.L-z1Va_pluginJumpBtn:focus-visible{outline:2px solid var(--dsw-alias-border-l4);outline-offset:1px}.L-z1Va_pluginName{appearance:none;min-width:0;font:inherit;color:var(--dsw-alias-label-primary);text-align:left;text-overflow:ellipsis;white-space:nowrap;cursor:pointer;background:0 0;border:0;padding:0;font-size:13px;font-weight:600;line-height:20px;text-decoration:none;overflow:hidden}.L-z1Va_pluginName:hover{color:var(--dsw-alias-button-primary-fill);text-decoration:underline}.L-z1Va_pluginDesc{color:var(--dsw-alias-label-secondary);font-size:12px;line-height:18px}.L-z1Va_pluginInstall{color:var(--dsw-alias-label-secondary);background:var(--dsw-alias-bg-layer-2);border:1px solid var(--dsw-alias-border-l1);white-space:nowrap;border-radius:8px;padding:6px 10px;font-family:ui-monospace,SFMono-Regular,Menlo,monospace;font-size:11px;line-height:16px;display:block;overflow-x:auto}.L-z1Va_pluginCopyBtn{appearance:none;font:inherit;cursor:pointer;background:var(--dsw-alias-label-primary);color:var(--dsw-alias-bg-layer-3);border:1px solid #0000;border-radius:8px;flex:none;padding:3px 12px;font-size:12px;line-height:1.5}.L-z1Va_pluginCopyBtn:hover{background:var(--dsw-alias-button-primary-hover)}.L-z1Va_pluginCopyBtn:focus-visible{outline:2px solid var(--dsw-alias-brand-primary);outline-offset:1px}@media (prefers-reduced-motion:reduce){.L-z1Va_card,.L-z1Va_switchTrack,.L-z1Va_switchThumb{transition:none}}";
		...
		var SideCardSection_module_css_default = {
			"pluginList": "L-z1Va_pluginList",
			"pluginEntryHead": "L-z1Va_pluginEntryHead",
			"cardTitle": "L-z1Va_cardTitle",
			"percentInput": "L-z1Va_percentInput",
			"pluginEntryActions": "L-z1Va_pluginEntryActions",
			"pluginModal": "L-z1Va_pluginModal",
			"cardDesc": "L-z1Va_cardDesc",
			"pluginJumpBtn": "L-z1Va_pluginJumpBtn",
			"cardMain": "L-z1Va_cardMain",
			"cardTop": "L-z1Va_cardTop",
			"switch": "L-z1Va_switch",
			"popupRows": "L-z1Va_popupRows",
			"grid": "L-z1Va_grid",
			"card": "L-z1Va_card",
			"group": "L-z1Va_group",
			"cardGear": "L-z1Va_cardGear",
			"cardIconChip": "L-z1Va_cardIconChip",
			"pluginEntries": "L-z1Va_pluginEntries",
			"typedInput": "L-z1Va_typedInput",
			"intro": "L-z1Va_intro",
			"desc": "L-z1Va_desc",
			"switchTrack": "L-z1Va_switchTrack",
			"pluginName": "L-z1Va_pluginName",
			"pluginInstall": "L-z1Va_pluginInstall",
			"pluginEntry": "L-z1Va_pluginEntry",
			"suffix": "L-z1Va_suffix",
			"rowText": "L-z1Va_rowText",
			"done": "L-z1Va_done",
			"control": "L-z1Va_control",
			"cardWithGear": "L-z1Va_cardWithGear",
			"pluginEmpty": "L-z1Va_pluginEmpty",
			"groupHeading": "L-z1Va_groupHeading",
			"row": "L-z1Va_row",
			"switchInput": "L-z1Va_switchInput",
			"typedInputNumber": "L-z1Va_typedInputNumber",
			"pluginDesc": "L-z1Va_pluginDesc",
			"pluginCopyBtn": "L-z1Va_pluginCopyBtn",
			"cardCheck": "L-z1Va_cardCheck",
			"error": "L-z1Va_error",
			"title": "L-z1Va_title",
			"popupRow": "L-z1Va_popupRow",
			"popupDialog": "L-z1Va_popupDialog",
			"pluginTopicBtn": "L-z1Va_pluginTopicBtn",
			"count": "L-z1Va_count",
			"switchThumb": "L-z1Va_switchThumb",
			"cardOn": "L-z1Va_cardOn",
			"section": "L-z1Va_section",
			"addCard": "L-z1Va_addCard"
		};
		//#endregion
