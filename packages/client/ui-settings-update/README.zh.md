...
- **非 overlay 冲突与脏工作区会拦住按钮** — apply 不会 stash、不会强制 reset。`src/overlay.ts` 中的 overlay 路径会还原为本 fork 版本；其余冲突仍会中止。
